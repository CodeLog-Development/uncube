...
<template>
  <div class="w-full h-13 m-0 bg-blue-500 flex px-3 flex-row">
    ...
    <div class="self-center flex-1 place-content-end flex flex-row space-x-3">
      <button class="w-fit text-white p-1 rounded-md hover:backdrop-brightness-110 active:backdrop-brightness-85"
        v-for="item in navItems" :key="item.name" @click="router.push(item.path)">
        {{ item.name }}
      </button>

      <div v-if="!store.isLoggedIn" class="space-x-3">
        <button class="w-fit text-white p-1 rounded-md hover:backdrop-brightness-110 active:backdrop-brightness-85"
          v-for="item in noUserNavItems" :key="item.name" @click="router.push(item.path)">
          {{ item.name }}
        </button>
      </div>
    </div>
